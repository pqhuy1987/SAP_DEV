...
namespace R_BCTC
{
    ...
    class AC_BALANCE_SHEET : UserFormBase
    {
        ...
        System.Data.DataTable Get_MenuUID_TonghopsoPS()
        {
            DataTable result = new DataTable();
            SqlCommand cmd = null;
            try
            {
                cmd = new SqlCommand("GET_MENUUID", conn);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@ReportName", "Bảng kê tổng hợp số phát sinh");
                cmd.CommandTimeout = 0;
                conn.Open();
                SqlDataReader rd = cmd.ExecuteReader();
                result.Load(rd);
        ...
        DataTable Is_TonghopsoPS(string pAccount)
        {
            DataTable result = new DataTable();
            SqlCommand cmd = null;
            try
            {
                cmd = new SqlCommand("AC_MAP_TABLE_COA", conn);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@Account", pAccount);
                cmd.CommandTimeout = 0;
                conn.Open();
                SqlDataReader rd = cmd.ExecuteReader();
                result.Load(rd);
