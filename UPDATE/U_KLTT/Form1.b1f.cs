...
namespace U_KLTT
{
    ...
    class Form1 : UserFormBase
    {
        ...
        {
            BubbleEvent = true;
            if (pVal.EventType == SAPbouiCOM.BoEventTypes.et_CHOOSE_FROM_LIST && pVal.FormTypeEx =="U_KLTT.Form1")
            {
        ...
        private string Get_MenuUID_KLTT()
        {
            ...
            {
                if (oMenus.Item(i).String == "KLTT - Khối lượng thanh toán")
        ...
        private void Button0_PressedAfter(object sboObject, SAPbouiCOM.SBOItemEventArg pVal)
        {
            ...
            try
            {
                ...
                while (count_try <= 10)
                {
                    if (oForm.Title == "")
                    {
                        oApp.SetStatusBarMessage("Can't catch active form - Try again "+(count_try + 1).ToString());
                        count_try++;
                        Thread.Sleep(1000);
                        oForm = Application.SBO_Application.Forms.ActiveForm;
                        Application.SBO_Application.Forms.ActiveForm.Select();
                    }
                    else
                    {
                        break;
        ...
        private void Button1_PressedAfter(object sboObject, SAPbouiCOM.SBOItemEventArg pVal)
        {
            try
            {
                if (Grid0.Rows.SelectedRows.Count == 1)
                {
                    ...
                    if (frm.Title == "")
                    {
                        oApp.SetStatusBarMessage("Can't catch active form");
                        return;
                    }
                    frm.Mode = SAPbouiCOM.BoFormMode.fm_FIND_MODE;
                    frm.Items.Item("1_U_E").Enabled = true;
